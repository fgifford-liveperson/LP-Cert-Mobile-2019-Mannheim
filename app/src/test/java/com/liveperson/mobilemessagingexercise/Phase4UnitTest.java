...
public class Phase4UnitTest {
    ...
    @Test
    public final void testMyAccountFragmentConversationOnInitSucceed() {
        final MyAccountFragment myAccountFragment = Mockito.mock(MyAccountFragment.class);
        Mockito.when(myAccountFragment.getApplication())
                .thenReturn(Mockito.mock(MobileMessagingExerciseApplication.class));
        Mockito.when(myAccountFragment.getSupportFragmentManager())
        ...
        Mockito.when(LivePerson.getConversationFragment(
                any(LPAuthenticationParams.class),
                any(ConversationViewParams.class)))
                .thenReturn(Mockito.mock(ConversationFragment.class));
        new MyAccountFragmentConversation(myAccountFragment, ApplicationStorage.getInstance())
                .onInitSucceed();
        PowerMockito.verifyStatic(LivePerson.class, Mockito.description(
                "TODO Phase 4: Get the LivePerson fragment"));
        LivePerson.getConversationFragment(
                any(LPAuthenticationParams.class),
                any(ConversationViewParams.class)
        );
        Mockito.verify(myAccountFragment, Mockito.description(
                "TODO Phase 4: Add the LivePerson fragment to the screen"))
                .getSupportFragmentManager();
        PowerMockito.verifyStatic(FirebaseInstanceId.class, Mockito.description(
                "TODO Phase 4: Retrieve the Firebase token to use"));
